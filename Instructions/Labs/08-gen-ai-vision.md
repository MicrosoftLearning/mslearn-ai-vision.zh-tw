...
1. 選取 [建立]****，並等待您的專案建立完畢，這包括您選取的 gpt-4o 模型部署。

    > <font color="red"><b>重要</b>：</font>視 gpt-4o 模型的可用配額而定，您可能會收到將模型部署至不同區域中資源的額外提示。 若發生這種情況，請使用預設設定執行此操作。 稍後在本練習中，您將<b><u>無法</u></b>使用預設專案端點 - 您必須使用模型特定的目標 URI。
...
1. 在聊天遊樂場頁面上的 [設定]**** 窗格中，確定已選取 **gpt-4o** 模型部署。
...
1. 輸入以下命令，編輯已提供的設定檔：
   ...
   code .env
    ...
    > <font color="red"><b>重要</b>：</font>如果您在專案的預設區域中部署 gpt-4o 模型，則可以使用專案的 [概觀] <b></b>頁面上的 <b>Azure AI Foundry</b> 專案或 <b>Azure OpenAI</b> 端點來連線至您的模型。 如果您的配額不足，並已將模型部署至另一個區域，請在 [模型 + 端點]<b></b> 頁面上選取您的模型，並使用模型的<b>目標 URI</b>。

1. 在程式碼檔案中，將 **your_project_endpoint** 預留位置取代為模型的適當端點，並將 **your_model_deployment** 預留位置取代為您指派給 gpt-4o 模型部署的名稱。
